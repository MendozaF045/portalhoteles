...
Phase 1 (folder structure, Express skeleton, SQLite schema), Phase 2 (core REST endpoints), Phase 3 (Destinos endpoints + simulated cache refresh), Phase 4 (WhatsApp reservation link + general contact form), Phase 5 (frontend scaffold + public Home, plus the banner destacado backend it needed), Phase 6 (frontend Registro/Login/password recovery, wired to the real backend, with a persisted session), Phase 7 (the real `/panel-hotel`: edit profile, room CRUD, activation, plus the `PUT /hotel/me` backend endpoint it needed, plus a visibility-rule bug fix found during that phase), Phase 8 (the public hotel profile `/:slug`: details, room list, WhatsApp reservation form, plus the `GET /public/hoteles/:slug` backend endpoint it needed), Phase 9 (the Super Admin panel `/admin` with a separate login at `/admin/login`: hoteles activos/inactivos with delete, manual hotel creation, full banner CRUD — pure frontend, every backend endpoint it needed already existed), and Phase 10 (`/destinos` with país/ciudad filters and `/contacto` with a confirmation screen — also pure frontend, endpoints already existed since Phases 3 and 4) are done — see `README.md` for phase status and `API.md` for full endpoint docs. **Every page in the spec is now built** — no more routed placeholders. **Read `SPEC.md` in full before writing any code**; it is the source of truth for scope, and any change of scope must be reflected there first, per the doc's own instructions.
...
- **`components/ProximamentePage.jsx` has no remaining callers** as of Phase 10 — every route has a real page now. It's still exported/kept around since it's a trivial, reusable "coming soon" shell; reach for it first if a future phase adds a new route that isn't ready yet, rather than writing a one-off placeholder.
- **`pages/Home.jsx` and `pages/Destinos.jsx` share the same "fetch twice" filter pattern**: once unfiltered (to derive the país/ciudad filter dropdown options, with ciudad cascading off the selected país) and once with the active filters (debounced) to render the grid/list. `components/HotelFilters.jsx` and `components/DestinoFilters.jsx` are near-identical (Destinos just drops the price fields) rather than one generic component — if you're about to add a third filtered-list page, consider extracting a shared filter hook/component instead of copying a third time.
...
- **Admin session is a fully separate context/storage from the hotel session, on purpose**: `context/AdminAuthContext.jsx` mirrors `AuthContext.jsx`'s shape and API 1:1 (`{ token, admin }`, `login`/`logout`/`isAuthenticated`) but uses its own `localStorage` key (`portalhoteles_admin_auth`) and its own React context. `components/AdminPrivateRoute.jsx` mirrors `PrivateRoute.jsx` but redirects to `/admin/login` and checks `useAdminAuth()`. This means a browser can legitimately hold both a hotel session and an admin session at once, each independent — matching the backend's non-interchangeable-role design (verified in testing: an admin token against `/hotel/me` and a hotel token against `/admin/hoteles/activos` both `403`). Don't merge these into one context "to reduce duplication" — the duplication *is* the isolation.
- **No 4th color was added for the admin panel's visual differentiation** (spec section 10 caps the palette at 3): `components/admin/*` and `pages/AdminLogin.jsx`/`AdminPanel.jsx` differentiate via an `.admin-badge` pill ("ADMIN"/"ACTIVO", reusing the existing accent color at higher contrast) and an `.admin-card`/`.admin-panel .panel-hotel__header` accent-colored border, not a new hue. If asked to make the admin area "more distinct," reach for contrast/weight/badges within the existing 3 colors before proposing a new one.
- **`components/admin/BannerForm.jsx` manages a dynamic array of image URL inputs** (`form.imagenes`, add/remove rows) fully controlled by React state — safe to key rows by array index here specifically because nothing reorders and every input's `value` is state-driven, not `defaultValue`, so index-key reuse on removal doesn't cause stale-display bugs. Reused as-is for both banner create and edit (same component, like `HabitacionForm`).
- **`pages/AdminPanel.jsx` follows the same "parent owns state, children call `onCambio`" pattern as `PanelHotel.jsx`**: it fetches hoteles (activos + inactivos in parallel) and banners once into local state, and passes an `onCambio`/`onCreado` callback down to `AdminHoteles`, `AdminAgregarHotelForm`, and `AdminBanner` so any mutation (delete hotel, add hotel, banner create/edit/delete/activate) triggers a re-fetch of the relevant list rather than local optimistic state — keep following this pattern for any new admin section rather than introducing per-component caching.
- **`pages/Contacto.jsx` swaps the whole page to a confirmation screen on success** (`enviado` boolean state), same pattern as `RestablecerPassword.jsx`, rather than an inline success banner above a now-pointless empty form — deliberate per the user's explicit ask for "un mensaje de confirmación al enviar." It ignores the backend's `dev_note` field (unlike `RecuperarPassword.jsx`, which surfaces it because that page needs it to continue a flow); Contacto has no follow-up step, so there's nothing useful to show the visitor beyond the plain `message`.
...
- `/destinos` — Destination content per country/city, cached periodically from external sources. **Never copy source text verbatim** — must be original summary + backlink to source. **Built**, backend and frontend both (`GET /api/public/destinos`, the "periodic cache" is currently simulated, not a real fetch — see the Destinos bullets above; frontend `pages/Destinos.jsx` + `components/{DestinoFilters,DestinoCard}.jsx`).
- `/contacto` — General platform contact form (not hotel-specific). **Built**, backend and frontend both (`POST /api/contacto`; frontend `pages/Contacto.jsx`).
...
- `/admin` — Super admin panel, including banner/featured-hotel management (9.1: image gallery, link, title, description; only one featured hotel active at a time). **Built**, backend and frontend both (`/api/admin/*` + `/api/auth/admin/login`; frontend `pages/AdminLogin.jsx` (route `/admin/login`) + `pages/AdminPanel.jsx` + `components/admin/{AdminHoteles,AdminAgregarHotelForm,AdminBanner,BannerForm}.jsx`, auth-gated via `AdminPrivateRoute` with its own session — see the admin-session bullet above).
