...
Phase 1 (folder structure, Express skeleton, SQLite schema), Phase 2 (core REST endpoints), Phase 3 (Destinos endpoints + simulated cache refresh), Phase 4 (WhatsApp reservation link + general contact form), Phase 5 (frontend scaffold + public Home, plus the banner destacado backend it needed), and Phase 6 (frontend Registro/Login/password recovery, wired to the real backend, with a persisted session and a protected `/panel-hotel` placeholder) are done — see `README.md` for phase status and `API.md` for full endpoint docs. Destinos, Contacto, the real hotel panel, and the admin panel are still routed placeholders (or, for the admin panel, not even that). **Read `SPEC.md` in full before writing any code**; it is the source of truth for scope, and any change of scope must be reflected there first, per the doc's own instructions.
...
- Hotel panel (post-login): datos generales (7.1), habitaciones (7.2, min 4), estado Activar/Desactivar (7.3). Backend built (`/api/hotel/*`). Frontend: `/panel-hotel` route exists and is auth-gated (`PrivateRoute`), but renders a placeholder (`pages/PanelHotel.jsx`) — the real panel is the next phase.
